...
function install-kube-manifests {
  ...
  local -r kube_addon_registry="${KUBE_ADDON_REGISTRY:-registry.cn-hangzhou.aliyuncs.com/google_containers}"
  if [[ "${kube_addon_registry}" != "registry.cn-hangzhou.aliyuncs.com/google_containers" ]]; then
    find "${dst_dir}" -name \*.yaml -or -name \*.yaml.in | \
      xargs sed -ri "s@(image:\s.*)registry.cn-hangzhou.aliyuncs.com/google_containers@\1${kube_addon_registry}@"
