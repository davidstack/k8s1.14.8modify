...
function prepare-kube-proxy-manifest-variables {
  ...
  local kube_docker_registry="registry.cn-hangzhou.aliyuncs.com/google_containers"
...
function compute-master-manifest-variables {
  ...
  DOCKER_REGISTRY="registry.cn-hangzhou.aliyuncs.com/google_containers"
  if [[ -n "${KUBE_DOCKER_REGISTRY:-}" ]]; then
...
spec:
  ...
  containers:
  - name: pv-recycler
    image: registry.cn-hangzhou.aliyuncs.com/google_containers/busybox:1.27
    command:
    - /bin/sh
    args:
    - -c
